...
if "ecoinvent 3.9.1_cutoff_ecoSpold02" in bw.databases:
    ...
    ei37cutoff = bw.SingleOutputEcospold2Importer(
        fpei37cut, "ecoinvent 3.9.1_cutoff_ecoSpold02", use_mp=False
    )
    ei37cutoff
...
if type_method == "PEF":
    ...
    methods = [method_key for method_key in bw.methods if filter_func(method_key)]
    ...
    methods = [
        [
            m
            for m in bw.methods
            if "Cumulative Energy Demand (CED)" in str(m) and "total" in str(m)
        ][0]
    ]
...
results_df = pd.pivot_table(
    results_df, index=["Method"], columns=["Name"], values="Score"
)
if type_method == "PEF":
    ...
    results_df = results_df.rename(
        index={
            "Climate Change No Lt": "GWP",
            "Ozone Depletion No Lt": "OD",
            "Human Toxicity: Carcinogenic No Lt": "HT",
            "Human Toxicity: Non-Carcinogenic No Lt": "HTNC",
            "Particulate Matter Formation No Lt": "PMF",
            "Ionising Radiation: Human Health No Lt": "IR",
            "Photochemical Oxidant Formation: Human Health No Lt": "POF",
            "Acidification No Lt": "TAP",
            "Eutrophication: Terrestrial No Lt": "TE",
            "Eutrophication: Freshwater No Lt": "FE",
            "Eutrophication: Marine No Lt": "ME",
            "Ecotoxicity: Freshwater No Lt": "FET",
            "Land Use No Lt": "LU",
            "Water Use No Lt": "WD",
            "Material Resources: Metals/Minerals No Lt": "MRD",
            "Energy Resources: Non-Renewable No Lt": "FD",
        }
    )
...
for i in range(len(factors_global)):
    ...
    norm_planet_bound_df.iloc[i] = (
        norm_planet_bound_df.iloc[i] / factors_planet_bound[i]
    )
...
with pd.ExcelWriter(
    "".join([path_result, filename_result]), engine="openpyxl", mode="a"
) as writer:
