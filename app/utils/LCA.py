...
def get_cache_dirs():
    """
    Returns a list of cache directories depending on the OS.
    """
    system = platform.system()
    if system == "Windows":
        user_profile = os.path.expandvars("%USERPROFILE%")
        return [os.path.join(user_profile, "AppData")]
...
def delete_cache(target_string):
    """
    Deletes folders in OS-specific cache directories that contain the specified `target_string` in their name.

    :param target_string: The string to search for in folder names.
    :raises ValueError: If no cache directories exist.
    """
    ...
    if not found_any:
        print("No matching cache folders found.")
