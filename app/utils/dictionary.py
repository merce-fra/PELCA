...
def _init_dic(path_input, name_input, lca):
    ...
    df_RM = read_excel_sheet(excel, "Cur. Maint. (replac. matrix)", header=None, skiprows=[0, 1, 2, 3], usecols=lambda x: x != 0)
    ...
    if dic["num_hourPerYear"] > 8784:
        raise ValueError("Error: Annual usage time exceeds the total number of hours in a year (8784 hours).")
    ...
    dic["selected_EI"] = get_value_from_df(df_stair, "Plot specific env. impact") - 3 #1 changed to 3
