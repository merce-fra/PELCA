...
def fig_to_svg(fig, fullfilename):
    if fig["type"] == "plotly":
        svg_bytes = pio.to_image(fig["plot"], format='svg')
...
class ControlsWidget(QWidget):
    ...
    def save_plot(self, fig, png_folder, html_folder, svg_folder, prefix="default_plot"): #save_plot(self, fig, png_folder, html_folder, prefix="default_plot"):
        try:
            # Créer les dossiers s'ils n'existent pas
            os.makedirs(png_folder, exist_ok=True)
            os.makedirs(html_folder, exist_ok=True)
            os.makedirs(svg_folder, exist_ok=True) #Ligne ajoutée
            ...
            # Sauvegarde en SVG
            svg_filepath = get_unique_filepath(svg_folder, safe_title, "svg")
            fig_to_svg(fig,svg_filepath)
            ...
            print(f"Plot saved successfully as PNG: '{png_filepath}', HTML: '{html_filepath}' and SVG: '{svg_filepath}'") #print(f"Plot saved successfully as PNG: '{png_filepath}' and HTML: '{html_filepath}'")
    ...
    def save_plots(self):
        ...
        folder_name = self.figs['plot_data']['dic']['inventory_name']
        if folder_path:
            # Sous-dossiers pour PNG, HTML et SVG
            png_folder = os.path.join(folder_path, "plots", "png")
            html_folder = os.path.join(folder_path, "plots", "html")
            svg_folder = os.path.join(folder_path, "plots", "svg")
            ...
            for fig in self.figs.get("plots", []):
                ...
                self.save_plot(fig, png_folder, html_folder, svg_folder, prefix=title)
    ...
    def save_selected_plot(self):
        ...
        if folder_path:
            ...
            png_folder = os.path.join(folder_path, "plots_" + str(folder_name), "png")
            html_folder = os.path.join(folder_path, "plots_" + str(folder_name), "html")
            svg_folder = os.path.join(folder_path, "plots_" + str(folder_name), "svg")
    ...
    def save_data(self):
        ...
        folder_path = QFileDialog.getExistingDirectory(self, "Save data to numpy array")
        folder_name = self.figs['plot_data']['dic']['inventory_name']
        if folder_path:
            try:
                # Création du sous-dossier "numpy"
                numpy_folder = os.path.join(folder_path, "numpy")
                #numpy_folder = os.path.join(folder_path, "numpy_"+str(folder_name))
    ...
    def save_data_excel(self):
        folder_path = self.figs['plot_data']['dic']['LCA_path']
        folder_path = QFileDialog.getExistingDirectory(self, "Save data to excel")
        folder_name = self.figs['plot_data']['dic']['inventory_name']
        if folder_path:
            try:
                # Création du sous-dossier "excel"
                excel_folder = os.path.join(folder_path, "excel")
                #excel_folder = os.path.join(folder_path, "excel_"+str(folder_name))
